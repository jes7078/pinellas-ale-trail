		...
		() => {
			if (beer.length) {
				const breweryBeer = beer.filter((beers) => {
					return beers.breweries.name === props.match.params.name
				})
			...
			<section className="breweryPageBeer">
				...
				<section className="featuredSection">
					...
					<section className="breweryPagePictureFooter">
						...
						<h1>Description: {localStyles[beerNumber] && localStyles[beerNumber].description}</h1>
						<h1>ABV: {localStyles[beerNumber] && localStyles[beerNumber].abv}</h1>
					</section>
				</section>
			</section>
			{/* Menu Section */}
